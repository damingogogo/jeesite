...
public class EmpUserController extends BaseController {
	...
	public String save(@Validated EmpUser empUser, String op, HttpServletRequest request) {
		...
		if (StringUtils.inString(op, Global.OP_ADD, Global.OP_EDIT) && subject.isPermitted("sys:empUser:edit")) {
			...
			if (empUser.getXinxiType().equals("1")){
				UserTeacher teacher = new UserTeacher();
				BeanUtils.copyProperties(empUser,teacher);
				userTeacherService.save(teacher);
			}else if (empUser.getXinxiType().equals("2")){
				UserJiaz jiaz = new UserJiaz();
				BeanUtils.copyProperties(empUser,jiaz);
				userJiazService.save(jiaz);
			}else if (empUser.getXinxiType().equals("3")){
				UserStudent student = new UserStudent();
				BeanUtils.copyProperties(empUser,student);
				userStudentService.save(student);
			}
		}
		...
		User user =empUser;
		List<Map<String, Object>> list = OfficetreeData("2");
		for (int i=0;i<list.size();i++){
			UserDataScope userDataScope = new UserDataScope();
			String s = String.valueOf(list.get(i).get("id"));
			userDataScope.setCtrlType("Office");
			userDataScope.setCtrlData(s);
			user.getUserDataScopeList().add(userDataScope);
		}
		// 设置为二级管理员身份
	...
	private List<Map<String, Object>> OfficetreeData(String s) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			map.put("name", StringUtils.getTreeNodeName(null, e.getViewCode(), name));
			map.put("title", e.getFullName());
			// 返回是否是父节点，如果需要加载用户，则全部都是父节点，来加载用户数据
			map.put("isParent", !e.getIsTreeLeaf() || StringUtils.inString(null, "true", "lazy"));
			mapList.add(map);
		}
		// 一次性后台加载用户，若数据量比较大，建议使用懒加载
		if (StringUtils.equals(null, "true") && idList.size() > 0) {
			List<Map<String, Object>> userList =
					treeData(null, idList.toArray(new String[idList.size()]),
							null, null, null, null, null, null);
			mapList.addAll(userList);
		}
		// 懒加载用户，点击叶子节点的时候再去加载部门（懒加载无法回显，数据量大时，建议使用 listselect 实现列表选择用户）
		if (StringUtils.inString(null, "lazy") && StringUtils.isNotBlank(null)) {
			List<Map<String, Object>> userList =
					treeData(null, new String[]{null},
							null, null, null, null, null, null);
			mapList.addAll(userList);
		}
		return mapList;
	...
	public String delete(EmpUser empUser) {
		...
		if (empUser.getXinxiType().equals("1")){
			UserTeacher teacher = new UserTeacher();
			BeanUtils.copyProperties(empUser,teacher);
			userTeacherService.delete(teacher);
		}else if (empUser.getXinxiType().equals("2")){
			UserJiaz jiaz = new UserJiaz();
			BeanUtils.copyProperties(empUser,jiaz);
			userJiazService.delete(jiaz);
		}else if (empUser.getXinxiType().equals("3")){
			UserStudent student = new UserStudent();
			BeanUtils.copyProperties(empUser,student);
			userStudentService.delete(student);
		}
...
//		model.addAttribute("roleList", roleService.findList(role));
	 ...
	 */
	@RequiresPermissions("user")
	@RequestMapping(value = "empUserSelectTp")
	public String empUserSelectTp(EmpUser empUser, String selectData, Model model) {
		String selectDataJson = EncodeUtils.decodeUrl(selectData);
		if (selectDataJson != null && JSONValidator.from(selectDataJson).validate()){
			model.addAttribute("selectData", selectDataJson);
		}
...
//		model.addAttribute("roleList", roleService.findList(role));
		model.addAttribute("empUser", empUser);
		return "modules/web/empUserSelectTp";
	}
