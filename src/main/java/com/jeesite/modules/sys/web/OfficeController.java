...
//        office1 =office;
        office1.setViewCode(office.getViewCode());
        office1.setOfficeName(office.getOfficeName());
        office1.setStatus(office.getStatus());
        office1.setOfficeCode(office.getOfficeCode());


        office1.setShuxing(office.getShuxing());
        office1.setParentType(office.getParentType());

        List<Office> list = officeService.findList(office1);
        return list;
    }
    ...
    public String form(Office office, Model model) {
        ...
        List<Office> classnum = new ArrayList<>();
        List<EmpUser> collect = new ArrayList<>();
        if (office.getOfficeCode() != null && office.getOfficeCode() != "null") {
            int stunum = 0;
            List<Office> scholllist = officeService.findList(new Office());
            ...
            if (scholllist.size() > 0) {
                List<Office> nianjList = new ArrayList<>();
                try {
                    nianjList = scholllist.stream().filter(s -> s.getParentCode().equals(finalOffice.getOfficeCode())).collect(Collectors.toList());
                } catch (Exception e) {

                }
                if (nianjList.size() > 0) {
                    office.getExtend().setExtendI1(Long.valueOf(nianjList.size()));
                    for (int i = 0; i < nianjList.size(); i++) {
                        String officeCode = nianjList.get(i).getOfficeCode();
                        try {
                            collect =
                        ...
                        List<Office> classList = new ArrayList<>();
                        try {
                            classList = scholllist.stream().filter(s -> s.getParentCode().equals(officeCode)).collect(Collectors.toList());
                        } catch (Exception e) {

                        }
                        if (classList.size() > 0) {
                            for (int cla = 0; cla < classList.size(); cla++) {
                                classnum.add(classList.get(cla));
                            }

                        }
                    }
                    office.getExtend().setExtendI2(Long.valueOf(classnum.size()));
                    if (classnum.size() > 0) {
                        for (int i = 0; i < classnum.size(); i++) {
                            String officeCode = classnum.get(i).getOfficeCode();
                            try {
                                collect =
                                        userList.stream().filter(s -> s.getEmployee().getOffice().getOfficeCode().equals(officeCode)).collect(Collectors.toList());
    ...
    public String save(@Validated Office office) {
        if (office.getExtend1() != null) {
            office.getExtend().setExtendS1(office.getExtend1());
        }
        List<Office> classnum = new ArrayList<>();
        List<EmpUser> collect = new ArrayList<>();

        if (office.getShuxing().equals("1")) {
            int stunum = 0;
            List<Office> scholllist = officeService.findList(new Office());
            List<EmpUser> userList = empUserService.findList(new EmpUser());
            if (!office.getOfficeCode().isEmpty()) {
                try {
                    collect =
                            userList.stream().filter(s -> s.getEmployee().getOffice().getOfficeCode().equals(office.getOfficeCode())).collect(Collectors.toList());
                } catch (Exception e) {
...
//                List<Office> nianjList = new ArrayList<>();
//                try {
//                    nianjList = scholllist.stream().filter(s -> s.getParentCode().equals(office.getOfficeCode())).collect(Collectors.toList());
//                } catch (Exception e) {
//
//                }
//
...
//            office.getExtend().setExtendI3((long) stunum);
        ...
        return renderResult(Global.TRUE, text("保存''{0}''成功", office.getOfficeName()));
    }
