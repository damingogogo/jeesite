 ...
 * @version 2023-02-13
 */
@Table(name="sys_baoming_stastic", alias="a", label="报名统计信息", columns={
		@Column(name="id", attrName="id", label="id", isPK=true),
		@Column(name="baoming_id", attrName="baomingId", label="报名id"),
		@Column(name="bt_type", attrName="btType", label="属性", comment="属性(1:报名;2:填报)"),
		@Column(name="create_by", attrName="createBy", label="创建者", isUpdate=false, isQuery=true,queryType = QueryType.LIKE),
		@Column(name="create_date", attrName="createDate", label="创建时间", isUpdate=false,queryType = QueryType.GTE,isQuery=true, isUpdateForce=true),
		...
		@Column(name="office_code", attrName="officeCode", label=""),
		...
		@Column(name="huifu_wenjian", attrName="huifuWenjian", isQuery=false),

	}, orderBy="a.update_date DESC"
)
@Data
public class BaomingStastic extends DataEntity<BaomingStastic> {

	private static final long serialVersionUID = 1L;
	private String baomingId;		// 报名id
	private String btType;		// 属性(1:报名;2:填报)
	private String officeCode;		// 扩展 String 1
	...
	private String zb;		// 扩展 String 8
	private Integer toupnum;
	private Integer tprnum;
	private User user;
	private BaomingGl baomingGl;
	...
	public BaomingStastic() {
		this(null);
	}

	public BaomingStastic(String id){
		super(id);
	}

	@NotBlank(message="报名id不能为空")
	@Size(min=0, max=100, message="报名id长度不能超过 100 个字符")
